...
const HOST = process.env.NEXT_PUBLIC_PUBLIC_APP_URL?.replace(/^https://?:\/\//, '');
...
export async function submitUrlsToIndexNow(urls: string[]): Promise<boolean> {
    if (!INDEXNOW_KEY || !HOST) {
        console.warn("[IndexNow] INDEXNOW_KEY or site HOST is not configured. Skipping submission.");
        return false;
    }
    ...
    try {
        console.log(`[IndexNow] Submitting ${payload.urlList.length} URL(s):`, payload.urlList);
        const results = await Promise.allSettled(submissionPromises);

        results.forEach((result, index) => {
            if (result.status === 'fulfilled') {
                console.log(`[IndexNow] Successfully submitted to ${SEARCH_ENGINES[index]} (Status: ${result.value.status})`);
            } else {
                console.error(`[IndexNow] Failed to submit to ${SEARCH_ENGINES[index]}:`, result.reason.message);
            }
        });
        ...
        console.error("[IndexNow] A critical error occurred during submission:", error);
        return false;
    }
}