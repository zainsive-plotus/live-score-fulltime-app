...
interface LiveOddsClientProps {
  initialOdds: any; // The data fetched from the server
  fixtureId: string;
}

// Client-side fetching function for polling
...
export default function LiveOddsClient({
  ...
  const { data: liveOdds, isLoading } = useQuery({
    ...
    initialData: initialOdds, // Use server-fetched data for the first render
    staleTime: 10 * 1000,
  ...
  if (!liveOdds) {
    return null; // Don't render anything if there are no odds
  }
