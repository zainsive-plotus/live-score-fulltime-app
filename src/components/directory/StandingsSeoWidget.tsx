...
export default async function StandingsSeoWidget({
  locale,
}: {
  locale: string;
}) {
  const t = await getI18n(locale);
  const seoTitle = t("about_standings_title"); // You might need to add this translation key, e.g., "About Football Standings"
  const seoText = t("standings_hub_page_description"); // Reusing the meta description for the body text
